...
export class LiveMixin extends Vue {
  ...
  registSocketHandle() {
    (this.socket as SocketIOClient.Socket).on(
      "message",
      (data: ReceiveMessage) => {
   ...
   */
  createConnection(to: User, trackHandle?: TrackHandle) {
    let cutPeerConn = new RTCPeerConnection(this.rtcConfig);
    if (trackHandle) {
      cutPeerConn.ontrack = (trackHandle as TrackHandle).bind(cutPeerConn, to);
    }

    cutPeerConn.onconnectionstatechange = e => {
      console.log("connection state change", cutPeerConn.connectionState);
    };
